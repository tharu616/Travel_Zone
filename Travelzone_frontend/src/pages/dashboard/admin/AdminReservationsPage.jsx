...
export default function AdminReservationsPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.delete(`/api/admin/reservations/${id}`); load(); }
  ...
  return (
    <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
      <div style={{ marginBottom: "1.5rem" }}>
        <h1 style={{ fontSize: "1.4rem", fontWeight: 900, color: "var(--tz-text)" }}>Hotel Reservations</h1>
        <p style={{ color: "var(--tz-text-muted)", fontSize: "0.875rem" }}>All hotel reservations across the platform</p>
      </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid var(--tz-border)" }}>
                {["Guest", "Hotel", "Room Type", "Check-In", "Check-Out", "Status", "Price", "Actions"].map(h => (
                  <th key={h} style={{
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={8} style={{ padding: "2rem", textAlign: "center", color: "var(--tz-text-muted)" }}>
                    Loading…
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={8} style={{ padding: "2rem", textAlign: "center", color: "var(--tz-text-faint)" }}>
                ...
                return (
                  <tr key={r.id} style={{ borderBottom: "1px solid var(--tz-border)" }}>
                    ...
                    {/* Price */}
                    <td style={{ padding: "0.875rem 1.25rem", fontSize: "0.875rem", fontWeight: 600, color: "var(--tz-text)" }}>
