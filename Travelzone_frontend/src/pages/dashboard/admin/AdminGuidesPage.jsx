...
export default function AdminGuidesPage() {
  ...
  const handleStatus = async (id, status) => {
    try { await api.put(`/api/admin/guides/${id}/status`, { status }); load(); }
    catch { setError("Failed to update status"); }
  };

  const handleDelete = async (id, name) => {
    if (!window.confirm(`Delete guide "${name}"?`)) return;
    try { await api.delete(`/api/admin/guides/${id}`); load(); }
    catch { setError("Failed to delete guide"); }
  };
  ...
  return (
    <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
      <div style={{ marginBottom: "1.5rem" }}>
        <h1 style={{ fontSize: "1.4rem", fontWeight: 900, color: "var(--tz-text)" }}>Guides</h1>
        <p style={{ color: "var(--tz-text-muted)", fontSize: "0.875rem" }}>Manage all registered guides</p>
      </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid var(--tz-border)" }}>
                {["Guide", "Location", "Languages", "Price/Day", "Rating", "Status", "Actions"].map(h => (
                  <th key={h} style={{
            ...
            <tbody>
              {loading ? (
                ...
                const sc = statusColors[g.status] || statusColors.INACTIVE;
                return (
                  <tr key={g.id} style={{ borderBottom: "1px solid var(--tz-border)" }}>
                    ...
                    <td style={{ padding: "0.875rem 1.25rem" }}>
                      ...
                      <div style={{ fontSize: "0.78rem", color: "var(--tz-text-muted)" }}>
                        {g.email}
                      </div>
                    </td>
                    ...
                    <td style={{ padding: "0.875rem 1.25rem" }}>
                      <span style={{ display: "flex", alignItems: "center", gap: "0.25rem", fontSize: "0.85rem", color: "#d97706" }}>
                        <Star size={13} fill="#d97706" /> {g.rating?.toFixed(1) ?? "—"}
                      </span>
                    </td>
